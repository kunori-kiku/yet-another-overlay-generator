...
## Gotchas
- **`PutTopology` stores bytes verbatim — no key stripping.** Both impls defensively copy the caller's JSON and persist it untouched (`memstore.go:216-231`, `filestore.go:404-438`). The "public-keys-only" labels in `store.go:95-97,309` and docs/spec/controller/persistence.md (its §"Stored shapes" and §"Zero-knowledge custody") are a contract on CALLERS, not a store-enforced guarantee — persistence.md's claim is doc drift. Actual sanitization happens upstream (see specs/controller-stage-promote.md / specs/controller-operator-api.md).
