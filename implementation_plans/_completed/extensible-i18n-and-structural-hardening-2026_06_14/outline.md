...
| 2026-06-14 | D15: plan-3.5b **HTTP-envelope granularity** LOCKED via a 6-Opus debate (`wtrdjdvls`: brainstorm×3 → critic×2 → synthesize) — a judgement-free Q0–Q3 "who-can-fix-it" rule (method-gate → sentinel-map → bucket-vs-dedicated by who-can-fix + does-detail-vary), ~31+ codes, deep compile-constraints coded at source / I/O+plumbing as buckets with log-only causes, a serial PR breakdown ending in the grep-gated shim deletion. Refinements made during execution: poll-timeout is a 204 (no `request_timeout` code); not-found split into `node_not_found`/`config_not_found` (no English `{resource}` leak into zh); `compile_failed`/`stage_failed`/`manifest_signature_invalid` minted; `5+6` collapsed into one handler_controller.go PR; the auth chokepoint fixed at the PRODUCER (`authenticateNode` returns `*apierr.Error`). | The fork shaped ~9 PRs; the owner authorized opus-debate for genuinely-unclear forks. See `plan-3.5b-2026_06_14.md`. |
| 2026-06-14 | D16: plan-8 (struct-frontend) = **no refactor**. The audit's PLAUSIBLE frontend struct items (P1 controllerStore "god-store", P2 bidirectional store coupling) were rated by the adversarial verifiers as real-but-minor with quantitative claims materially wrong and the core coupling-severity REFUTED. A 177-field-store split / decoupling is a large, risky change whose justification was refuted, and the i18n/error debt it cited was already cleared by plan-1/2/3.5. | Decision rule = weigh value vs risk; refactoring on refuted evidence is negative-EV. The lazy-getState store cycle is a deliberate, documented runtime-only seam, not a defect. |
| 2026-06-14 | D17: **Key-custody edge cases** (owner asks): (a) local-import of a pubkey-only file (e.g. a controller export) stranded every node in GenerateKeys case (b) → clear ONLY stranded pubkey-only nodes on local import + local→controller (`clearStrandedKeys`/`switchToController`), never valid keypairs (avoids the accidental-round-trip data-loss footgun); (b) controller-import keeps no design keys at all (`dropAllKeys`) since the agent registry is authoritative. | Deployment was already safe (`enrolledSubgraph` stamps agent-registry pubkeys + drops unenrolled, so imports never reach a deploy); the change removes a per-node un-pin chore (a) and confusing non-authoritative metadata (b). [[controller-mode-redesign-decisions]] |
| 2026-06-14 | D18: **close-phase** — subject closed **DELIVERED** (owner-classified). All closure criteria pass (no `txt(`/`STRINGS` call sites, `go test ./...`+`vet` green, no operator-facing Chinese on any channel, mode boundary leak-proof, architecture+migration note written). CLOSURE.md written; folder archived to `_completed/`; STATUS.md regenerated; memory [[i18n-error-envelope-shipped]] kept as-is. | Everything executable shipped (PRs #70–#95, each reviewed); the two-node controller smoke is a manual hardware verification owed, not a code gap. |
| 2026-06-14 | D19: **specs/ = Light edit** (owner-chosen, Phase 4.6) — bump `last-verified` + a one-line "errors now coded via the apierr envelope" note on the affected components, NOT a `/refresh-specs`. | The i18n/error-envelope work is an error-message localization refactor; it changes how error responses are shaped, not the architectural story (no component added/removed/split). |
...
| plan-11 (controller-mode boundary UI gating) | **done** | [#80](https://github.com/kunori-kiku/yet-another-overlay-generator/pull/80) | gate Topbar import/export/flush to local + surface Save; mode-split SecurityPage; CompilePreview local-only; Connection/Bootstrap controller-only; RequireControllerMode route guard; gate pin-key (NodeEditor + NodeForm) + SSH editor. Reviewed (5 confirmed fixed incl. the NodeForm twin). |
| plan-12 (backend mode-awareness) | **done** | [#81](https://github.com/kunori-kiku/yet-another-overlay-generator/pull/81) | air-gap compute routes behind operatorAuth in controller mode (gateAirgap); /api/health public; validate still usable. Tests: air-gap open / 401-without-auth / pass-with-bearer / cookie+CSRF / health==200. |
| plan-3.5 (remaining backend strings) | **done** | [#82](https://github.com/kunori-kiku/yet-another-overlay-generator/pull/82) (3.5a) · [#83](https://github.com/kunori-kiku/yet-another-overlay-generator/pull/83)–[#93](https://github.com/kunori-kiku/yet-another-overlay-generator/pull/93) (3.5b) · [#92](https://github.com/kunori-kiku/yet-another-overlay-generator/pull/92) (3.5c) | **3.5a** validator channel (89 coded findings + tValidationError). **3.5b** HTTP-envelope channel coded end-to-end (compile/render/export · handler.go · full controller surface · auth cluster; ~80 codes total) then the `writeError`/`CodeLegacyUncoded`/`WithMessage` shim DELETED (grep-gated). **3.5c** deploy-artifact (install-script + self-extracting installer) Englishized. Perpetual CJK gate over internal/{api,compiler,render,artifacts,renderer}. Granularity strategy locked by a 6-Opus debate; every PR independently reviewed. |
| plan-7 (struct-backend) | **done** | [#94](https://github.com/kunori-kiku/yet-another-overlay-generator/pull/94) | P7 reserved-id helper (`isReservedNodeID`, 2-site dedup). P2 (render.go mixed-language) resolved by the i18n work; P3/P5/P6 rated overstated/refuted — no action (see Decisions log). |
| plan-8 (struct-frontend) | **done (triage: no refactor)** | — | The struct-frontend PLAUSIBLE items (P1 god-store, P2 bidirectional coupling) were rated by the adversarial verifiers as "real-but-minor / quantitatively wrong / core REFUTED". A 177-field-store split is a large, risky refactor with refuted justification; the i18n/error debt it cited was already cleared by plan-1/2/3.5. Decision: no refactor (see Decisions log). |
| plan-9 (cross-cutting + docs + closure) | **partial** | [#95](https://github.com/kunori-kiku/yet-another-overlay-generator/pull/95) | Outline status refresh + plan-8 triage decision + i18n/error-envelope architecture & migration note (this closeout). Test-coverage gaps addressed inline across the plan-3.5b PRs. **Owner-owned closure remaining:** formal `/close-phase` (status classification, archive move, memory note, STATUS.md, per-component `specs/` refresh) + the long-owed two-node controller smoke. |
| key-custody side-quests (owner edge-case asks, 2026-06-14) | **done** | [#89](https://github.com/kunori-kiku/yet-another-overlay-generator/pull/89), [#91](https://github.com/kunori-kiku/yet-another-overlay-generator/pull/91) | #89 local-import stranded pubkey-only clearing (`clearStrandedKeys` — keeps valid round-trip keypairs) + symmetric `switchToController`; #91 controller-import drops ALL design keys (`dropAllKeys`) since agents are authoritative (enrolledSubgraph stamps registry pubkeys / drops unenrolled — imports never reach a deploy). Both reviewed. |
