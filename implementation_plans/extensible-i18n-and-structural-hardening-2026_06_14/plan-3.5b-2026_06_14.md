...
| **3.5b-5** | **handler_controller.go FULL** (collapsed old 5+6 — one file, one concern, ~300-line diff avoids a file-wide `replace_all` crossing a PR boundary): enrollment / config / poll / apply / topology / stage / promote / nodes / revoke / audit / versions / rekey / operator-cred / keystone-sign | apierr.go(+test), handler_controller.go, handler.go (drop dead isBodyTooLarge), en/zh | 110 | 19 | api tests incl. topology_custody + enrollment + deploy/keystone perpetual; bad token→401, dup key→409, bad JSON→400 without regressing custody; staged-mismatch + no-pinned-credential coded; build green; bijection+CJK pass. **Custody branch (`errors.As(syn)`→CodeCustodyPrivateKey) untouched.** Not-found split into `node_not_found`/`config_not_found` (distinct codes, no English `{resource}` leak into zh). |
