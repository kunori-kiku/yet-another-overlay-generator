...
// compareVersions compares two SemVer-ish version strings, returning -1, 0, or 1. It is the
// agent self-update ordering primitive (plan-9): a documented + tested comparator rather than
// a new dependency (golang.org/x/mod/semver would also do, but dependency-minimalism wins for
// this small need — PRINCIPLES.md). It implements enough of SemVer 2.0.0 precedence for the
// floor / downgrade / min-version checks:
//
...
//
// The EMPTY string is the MINIMAL sentinel — below every real version, equal only to itself —
// so a legacy agent that reports no version is treated as below any floor and self-updates
// rather than being frozen out (plan-9 Addition 4a).
