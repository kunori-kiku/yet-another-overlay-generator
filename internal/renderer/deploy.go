...
#
# Usage: bash deploy-all.sh [--clean] [--uninstall] <path-to-artifacts.zip>
#
# Options:
...
#   --uninstall  Completely remove the overlay from all nodes. Uploads the
#                installer and runs it with --uninstall on each SSH-configured node.
#
...
if [ -z "$ARTIFACTS_ZIP" ]; then
    echo "Usage: $0 [--clean] [--uninstall] <path-to-artifacts.zip>"
    echo ""
    echo "Options:"
...
if [ ! -f "$INSTALLER" ]; then
    ...
    if ! ssh%s %s "echo ok" >/dev/null 2>&1; then
        ...
        if [ "$CLEAN" -eq 1 ] && [ "$UNINSTALL" -eq 0 ]; then
...
CLEAN_EOF
        ...
        if %s "$INSTALLER" %s:/tmp/%s; then
            if ssh%s %s "sudo bash /tmp/%s $INSTALL_ARGS && rm -f /tmp/%s"; then
                if [ "$UNINSTALL" -eq 1 ]; then
...
# Generated by Overlay Network Config Orchestrator
#
# Usage: .\deploy-all.ps1 -ArtifactsZip <path-to-artifacts.zip> [-Clean] [-Uninstall]
#
# Options:
#   -Clean      Remove ALL existing WireGuard interfaces (wg*) before deploying.
#               Use this when switching from a single-interface (wg0) layout to
    ...
    if (-not (Test-Path $Installer)) {
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            if ($LASTEXITCODE -ne 0) {
                ...
                & ssh%s %s "sudo bash /tmp/%s$InstallArgs && rm -f /tmp/%s"
                if ($LASTEXITCODE -ne 0) {
