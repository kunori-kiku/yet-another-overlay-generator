...
//
// Scoped to *.conformance.test.ts (the cross-language pins) PLUS the pure-TS compiler unit tests
// (src/compiler/**/*.test.ts — the leaf-primitive KAT/CIDR sweeps the TS port ships, plan-4). Both
// families are dependency-free Node-environment unit pins that read fixtures from disk; neither needs
...
export default defineConfig({
  test: {
    include: ['src/**/*.conformance.test.ts', 'src/compiler/**/*.test.ts'],
    environment: 'node',
