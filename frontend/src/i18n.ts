...
export const STRINGS = {
  ...
  skipToContent: ['跳到主内容', 'Skip to content'] as const,
  ...
  chClearHistory: ['清空历史', 'Clear History'] as const,
  chExposureAudit: ['全局暴露审计', 'Global Exposure Audit'] as const,
  chExposedNodes: ['暴露节点（公网 / 中继）', 'Exposed Nodes (Public / Relays)'] as const,
  chNoExposedNodes: ['无暴露节点', 'No exposed nodes'] as const,
  chListensOnPort: ['监听端口', 'Listens on port'] as const,
  chInboundPaths: ['入站允许路径', 'Inbound allowed paths'] as const,
  chNetworkStats: ['网络统计', 'Network Statistics'] as const,
  chTotalNodes: ['节点总数', 'Total Nodes'] as const,
  chEncryptedEdges: ['加密链路', 'Encrypted Edges'] as const,
  chCurrentChecksum: ['当前校验和', 'Current Checksum'] as const,
  chCompilationHistory: ['编译历史', 'Compilation History'] as const,
  chCurrentWorkingState: ['当前工作状态', 'Current Working State'] as const,
  chReadyToExport: ['可导出', 'Ready to export'] as const,
  chSnapshot: ['快照', 'Snapshot'] as const,
  chNoHistory: ['暂无历史。请先编译项目。', 'No history available. Compile the project first.'] as const,
  chConfigDiff: ['配置差异', 'Configuration Diff'] as const,
  chCurrentVsSnapshot: ['（当前 vs 快照）', '(Current vs Snapshot)'] as const,
  chSelectFileToDiff: ['选择要对比的文件...', 'Select a file to diff...'] as const,
  chNoWgConfigs: ['无 wg 配置', 'no wg configs'] as const,
  chSelectFromDropdown: [
    '从下拉框选择文件以查看差异。',
    'Select a file from the dropdown to view differences.',
  ] as const,
  chFileNotInCurrent: ['当前编译中未找到该文件。', 'File not found in current compilation.'] as const,
  chFileNotInBoth: ['该文件在两个状态中都不存在。', 'File does not exist in both states.'] as const,
