  ...
  const renderDiff = (oldText: string, newText: string) => {
    ...
    const renderUnchangedPart = (value: string, key: number) => {
      ...
      if (contentLines.length <= DIFF_CONTEXT_LINES * 2 + 1) {
        return (
          <span key={key} className="text-[var(--content)]">
      ...
      return (
        <span key={key} className="text-[var(--content)]">
          ...
          <div className="text-[var(--content-muted)]">
    ...
    return (
      <div className="font-mono text-xs whitespace-pre pl-2">
        {changes.map((part, index) => {
          ...
          const color = part.added ? 'bg-[var(--success-bg)] text-[var(--success)]' : part.removed ? 'bg-[var(--danger-bg)] text-[var(--danger)]' : 'text-[var(--content)]';
  ...
  return (
    ...
    <div className="flex flex-col p-3 sm:p-6 space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold text-[var(--content)]">{t(language, 'compileHistoryTitle')}</h2>
        <button onClick={clearHistory} className="px-3 py-1.5 bg-[var(--danger-solid)] hover:bg-[var(--danger-solid)] text-[var(--danger-solid-fg)] text-sm rounded">{t(language, 'chClearHistory')}</button>
      </div>

      {/* Global Audit Summary */}
      <section className="bg-[var(--surface-elevated)] border border-[var(--hairline)] p-4 rounded-lg">
        <h3 className="text-lg font-semibold mb-3 text-[var(--warning)]">🛡️ {t(language, 'chExposureAudit')}</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <h4 className="text-sm text-[var(--content-muted)] mb-2">{t(language, 'chExposedNodes')}</h4>
            <p className="text-xs text-[var(--content-muted)] mb-2 italic">
              {auditNodesAreBackendInferred
                ? t(language, 'auditView.basedOnBackendInferred')
                : t(language, 'auditView.preCompileEstimateFrom')}
            </p>
            {exposedNodes.length === 0 ? <span className="text-xs text-[var(--content-muted)]">{t(language, 'chNoExposedNodes')}</span> : (
              <ul className="text-sm space-y-1">
                {exposedNodes.map(n => {
                  const inboundEdges = edges.filter(e => e.to_node_id === n.id);
                  return (
                    <li key={n.id} className="text-[var(--content)]">
                      <strong>{n.name}</strong> ({n.role}) - {n.overlay_ip}<br />
                      <span className="text-[var(--content-muted)] text-xs pl-2">{t(language, 'chInboundPaths')}: {inboundEdges.length}</span>
                    </li>
                  )
                })}
              </ul>
          ...
          <div>
            <h4 className="text-sm text-[var(--content-muted)] mb-2">{t(language, 'chNetworkStats')}</h4>
            <ul className="text-sm text-[var(--content)] space-y-1">
              <li>{t(language, 'chTotalNodes')}: {nodes.length}</li>
              <li>{t(language, 'chEncryptedEdges')}: {edges.length}</li>
              <li>{t(language, 'chCurrentChecksum')}:{currentResult ? <span className="font-mono text-xs ml-2 bg-[var(--surface-sunken)] p-1 rounded break-all">{currentResult.manifest.checksum}</span> : ' N/A'}</li>
            </ul>
          </div>
        </div>
      </section>
      ...
      <section className="flex flex-1 gap-6 min-h-[400px]">
        {/* History List */}
        <div className="w-1/3 flex flex-col bg-[var(--surface-elevated)] border border-[var(--hairline)] p-4 rounded-lg">
          <h3 className="text-lg font-semibold mb-3 text-[var(--info)]">📜 {t(language, 'chCompilationHistory')}</h3>
          <div className="flex-1 overflow-y-auto space-y-2">
            {currentResult && (
              <div
                className={`p-3 rounded cursor-pointer border ${selectedHistoryId === null ? 'border-[var(--accent)] bg-[var(--control-hover)]' : 'border-[var(--hairline)] bg-[var(--control)] hover:bg-[var(--control-hover)]'}`}
                onClick={() => setSelectedHistoryId(null)}
              >
                <div className="font-semibold text-sm">{t(language, 'chCurrentWorkingState')}</div>
                <div className="text-xs text-[var(--content-muted)]">{t(language, 'chReadyToExport')}</div>
              </div>
            )}
            ...
            {history.map((h, i) => (
              <div
                key={h.id}
                className={`p-3 rounded cursor-pointer border ${selectedHistoryId === h.id ? 'border-[var(--accent)] bg-[var(--control-hover)]' : 'border-[var(--hairline)] bg-[var(--control)] hover:bg-[var(--control-hover)]'}`}
                onClick={() => setSelectedHistoryId(h.id)}
              >
                <div className="font-semibold text-sm">{t(language, 'chSnapshot')} {history.length - i}</div>
                <div className="text-xs text-[var(--content-muted)]">{new Date(h.timestamp).toLocaleString()}</div>
                <div className="text-xs text-[var(--content-muted)] font-mono mt-1 w-24 truncate">{h.compileResult.manifest.checksum}</div>
              </div>
            ))}
            {history.length === 0 && !currentResult && <div className="text-sm text-[var(--content-muted)] italic">{t(language, 'chNoHistory')}</div>}
          </div>
        </div>

        {/* Diff View */}
        <div className="w-2/3 flex flex-col bg-[var(--surface-elevated)] border border-[var(--hairline)] rounded-lg overflow-hidden">
          <div className="p-3 border-b border-[var(--hairline)] bg-[var(--surface-elevated)] flex justify-between items-center shrink-0">
            <h3 className="text-lg font-semibold text-[var(--content)]">🔍 {t(language, 'chConfigDiff')} {selectedHistoryId && t(language, 'chCurrentVsSnapshot')}</h3>
            <select
              className="bg-[var(--control)] border border-[var(--hairline)] text-sm rounded px-2 py-1"
          ...
          <div className="flex-1 p-4 overflow-y-auto bg-[var(--surface-sunken)]">
            {!selectedNodeFileId ? (
              <div className="text-center text-[var(--content-muted)] mt-20">{t(language, 'chSelectFromDropdown')}</div>
            ) : selectedHistoryId === null ? (
              <pre className="font-mono text-xs text-[var(--content)] whitespace-pre-wrap">{currentText || t(language, 'chFileNotInCurrent')}</pre>
            ) : (
              <div>
                {!oldText && !currentText ? (
                  <div className="text-[var(--content-muted)] italic">{t(language, 'chFileNotInBoth')}</div>
                ) : (
