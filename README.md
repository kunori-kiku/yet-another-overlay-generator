...
- **Fleet Management (2.0):** Per-node fleet pages with status detail, single-use enrollment tokens minted from the panel, a compile-history/audit view, and manual fleet-wide WireGuard key rotation (Roll keys). Routine authenticated `/report` and `/telemetry` updates change Fleet state without appending durable audit rows; legacy report rows remain in the verified raw chain but are hidden from the normal audit table.
...
- **Signed, Charted Telemetry (2.0):** From **Fleet**, configure typed ICMP echo, TCP-connect, and fixed-GET HTTP(S) URL probes, plus opt-in automatic Linux disk/filesystem/GPU observation. A URL probe compares the first response with one expected status (default 200): latency and availability are charted, while the actual status remains latest-result text. Device inventory is live-only; filesystem use, disk throughput/busy time, GPU utilisation, and VRAM use reuse the shared bounded history charts. Successor policy is checksum/signature-covered; rolling upgrades use an explicit upgrade-agents-first then normal-deploy sequence. Live results, device data, capabilities, and fetched history are never persisted in browser storage. See [`docs/spec/operations/active-telemetry.md`](docs/spec/operations/active-telemetry.md).
