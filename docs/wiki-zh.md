...
保持有界。资源流与所选探测流共同使用全局 1000 桶预算，Fleet 只获取当前精确选中的探测目标，因此大时间范围
传输的是紧凑汇总而非完整保留序列；实际生效的步长会被回传并显示。若显式选择的分辨率为了满足全局响应预算而被
控制器放宽，Fleet 会明确提示；24 小时与 7 天范围的横轴也会显示日期，避免重复的时钟时间产生歧义。选择
...
即**30 秒心跳下约 7 天**。填 **0 即禁用**历史。对于宽度可变的探测记录，每节点独立的 128 MiB 文件安全上限
优先于记录目标；压实以流式方式进行，查询也只扫描最新的有界后缀。历史**绝不阻塞心跳**（心跳绝不写盘），也不会
定格：心跳是唯一数据源，且 agent 会在每次 apply 后立即推动新样本。

**签名主动遥测。** 在 **Fleet（集群）** 的节点详情页中，可以配置多条 ICMP Echo 或 TCP 连接检测，并在同一位置
查看最近结果以及延迟/可用性历史。失败尝试会作为真实故障保留，绝不会伪装成 0 毫秒延迟；未上报区间仍显示为空缺。
每条只需一个必填的目标 `host`，可填写 IP 字面量或 DNS 主机名；没有单独或强制的 DNS 字段。TCP
