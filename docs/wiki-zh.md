...
跳过判定与身份标识，因此这个计数与实际结果一致。**Deploy anyway（照常部署）** 只是一个狭窄的兼容逃生口：
仅当旧控制器因根本没有预览路由而返回 404/405，且当前拓扑仍与旧版 v1 策略兼容时，面板才会提供它。结构化
校验、安全、存储、运行或网络故障一律阻断部署。含后继策略（URL 探测或自动设备遥测）的拓扑也禁止绕过，因为
旧控制器无法安全编译或执行该策略。
...
版本**用于恢复，并用一份只追加、哈希链式的**审计日志**记录 enroll/revoke/stage/promote/rekey 等有意义的
运营者或安全事件。常规、已鉴权的 `/report` 与 `/telemetry` 只更新已应用 generation/Fleet 健康状态，不追加
持久审计行。旧控制器已经写入的 `report` 行会为兼容性保留在原始 API 与哈希链中：面板先验证完整链，再仅从
日常表格中隐藏这些旧行；不会删除或改写任何链记录。
...
(3) 通常等待每个“rotating keys（轮换密钥中）”徽章清除；(4) 一次普通的 Deploy 用全部新公钥重新编译。也可以在
确认提示后提前 Deploy；此时每个节点使用当前已登记的密钥，但迟到节点完成轮换后还需再次 Deploy（或对永不返回的
节点显式取消 rekey）。代价是一次短暂的滚动式逐链路抖动。
...
5. **report** —— `POST /report` 记录已应用的 generation/校验和/健康（尽力而为），但不追加常规持久审计事件。
...
限制在 3 次尝试，之后 agent 回滚到保存的 `.bak`。替换后的健康请求若失败，agent 会保留 breadcrumb 与 `.bak`，
并在下一次受监管启动时重试；单次瞬时 `EOF` 不会放弃该目标版本。健康检查成功后，新版本才进入**试用期
（probationary）**，且**防降级下限**只有在一次完整、干净的循环之后才推进。
...
状态严格分离；该心跳与常规 `/report` 状态都不会追加到持久审计链。
...
绘成图表：CPU %、内存使用率 %，以及负载均值。你选择一个**时间范围**与**分辨率**，服务器把保留的观测
聚合进各个桶（每桶给出平均/最小/最大），并**略去空桶**——数据中的空缺（节点离线、历史刚启用）在
图上仍是空缺，绝不伪造 0。过细的步长会被限制在约 1 秒的下限，而在大范围上过细的步长会被自动放宽，使图表
保持有界。资源、精确选中的探测流以及精确选中的设备序列共同使用全局 1000 桶预算；Fleet 不会为了画一张图而
取回所有探测或设备。因此大时间范围传输的是紧凑汇总而非完整保留序列；实际生效的步长会被回传并显示。若显式
选择的分辨率为了满足全局响应预算而被控制器放宽，Fleet 会明确提示；24 小时与 7 天范围的横轴也会显示日期，
避免重复的时钟时间产生歧义。选择
**自动（Auto）**分辨率时，服务器优先采用最近一次上报的心跳间隔；若没有，则从观测时间差中取抗离线空档的较低
中位数（并避免把部署后立即推动的短间隔误认为长期节奏），下限为 30 秒。缺失某项指标的桶始终显示为空缺。为避免
健康样本恰好落在相邻桶边界两侧时造成假断线，允许跨过一个空的有效桶；更长的连续空缺会让曲线断开，不会跨过离线
区间强行连线。

**保留量可配置且有字节上限。** 控制器 **Settings（设置）** 中的每节点记录目标限定逻辑历史；默认约 20160 条，
即**30 秒心跳下约 7 天**。填 **0 即禁用**历史。对于宽度可变的探测/设备记录，每节点独立的 128 MiB 文件安全
上限优先于记录目标；压实以流式方式进行，查询也只扫描最新的有界后缀。历史**绝不阻塞心跳**（心跳绝不写盘），也不会
定格：心跳是唯一数据源，且 agent 会在每次 apply 后立即推动新样本。

**签名主动遥测与自动设备观测。** 在 **Fleet（集群）** 的节点详情页中，可以配置多条有明确类型的 ICMP Echo、
TCP 连接或 HTTP(S) URL 检测，并在同一位置查看当前结果及精确序列历史。ICMP/TCP 使用一个 `host`，可填写 IP
字面量或 DNS 主机名——没有单独或强制的 DNS 字段；TCP 另需端口。URL 是独立类型，包含一个精确的绝对 URL 与
一个预期状态码（默认 200）。它只发送一次固定 GET、不跟随重定向，并比较首个响应。实际返回码只作为最新结果文字，
不画图；不匹配会记作可用性失败，但保留测得的延迟。延迟与可用性都有图表，传输失败绝不伪造 0 毫秒，未上报区间
仍是真实空缺。

保存只更新控制器设计草稿，**Deploy（部署）** 才是签名与启用边界。仅含 ICMP/TCP 的既有设计继续使用严格、
字节兼容的 `telemetry.json` v1。含 URL 或自动设备策略时，使用与它互斥的签名
`telemetry-policy.json` v2，并要求当前已鉴权的 agent 能力证据。滚动升级刻意分两次部署：先选
**Upgrade agents first（先升级 agent）**，在保留完整已保存草稿的同时部署内存中的 v1 投影。已配置的签名滚动
更新只会升级其覆盖的节点；未配置或覆盖不完整时只显示非阻断警告，未覆盖的 agent 必须通过带外方式升级。等所有
受影响 agent 上报新的所需能力后，再普通 Deploy 以启用完整 v2 策略。能力不足的 agent 会在主机变更前拒绝，失败
候选继续保留上一个活动策略。所有带主动遥测策略的部署都需要已固定的离线 Keystone，agent 在启用前还会再次验证成员资格。

同一 Fleet 页面还提供可选的 **Automatically discover eligible disks and GPUs（自动发现符合条件的磁盘与 GPU）**
策略。在 Linux 上，agent 会有界地发现块设备、已挂载文件系统与 GPU；NVIDIA 使用固定、有界的 `nvidia-smi`
查询，AMD 使用文档化的 sysfs 指标，不可用或不支持的提供者会保留为明确的 inventory 状态。分类信息
`device_inventory` 仅实时显示；文件系统使用率、磁盘读写吞吐与 busy 百分比、GPU 利用率和显存使用率属于数值型
`device_samples`，复用同一套历史图表。真实空闲的 0 保持为 0；缺失、首次、计数器重置或不支持的读数保持为图表
空缺。Fleet 只在同一全局桶预算内查询所选 `(kind, device_id)` 序列。实时探测结果、设备 inventory/读数、能力证据、
选择状态和已获取历史都不会写入浏览器持久化。ICMP 使用进程内原始套接字并明确报告权限不足；TCP 不依赖外部
`tcping` 包。详见[主动遥测规范](spec/operations/active-telemetry.md)。

**Fleet 实时刷新反馈。** 开启 **实时** 后会立即刷新，之后每次请求完成 10 秒再发起下一次，因此控制器较慢时不会
叠加请求。页面会显示当前是否正在刷新、距离上次成功更新多久以及下次刷新倒计时；标签页隐藏时暂停，重新可见时立即
刷新。若成功更新持续中断，状态会变为延迟/过期；刷新图标只在真正进行网络请求时旋转。
这个高频路径只获取节点观测快照，不会每 10 秒重复下载完整审计链、设置或 Keystone 状态；它还使用独立的 Fleet
成功时间，因此保存无关拓扑不会让遥测看起来刚刚更新。当该节点的 `last_seen` 前进时才重新获取历史；短暂读取失败时
保留上次成功图表，并显示独立的更新中、上次成功或警告反馈。即使离线节点的 `last_seen` 不再变化，也可显式重试。
...
`update-topology`、`stage`、`compile-preview`、`deploy-preview`、`promote`、
...
`report`、`telemetry`、`rekey` 需要各节点 bearer 令牌。`report` 更新已应用 generation/校验和状态，
`telemetry` 更新 conditions、metrics 与 last-seen，但不改变部署托管字段；两者都不追加常规持久审计行。
旧版 `report` 行继续保留在原始、已验证的链/API 中，日常审计表只会在完整链验证后隐藏它们。
...
| **可部署就绪子图** | 控制器实际渲染的已注册受管节点、有效手动节点，以及它们之间符合条件的边。 |
