...
- **Phase B — `ReconcileSelfUpdatePromote`** runs after the client + pinned key exist. When the
  ...
  NOT yet advanced); a failure returns startup failure while retaining both the breadcrumb and
  `.bak`, so systemd restarts the target and retries under Phase A's durable attempt counter. One
  transient `EOF`, timeout, reset, or HTTP failure therefore cannot poison the release; only
  exhausting the common three-attempt ceiling rolls back + abandons. A breadcrumb that is ALREADY
  `Confirmed` on boot **resumes probation** (it does NOT immediately roll back — a benign host
  reboot during the short probation window must not falsely abandon a healthy binary); a genuinely
  crashing binary never completes a cycle to finalize and is caught by the same cap. This closes
  the daemon-only-crash-after-promote brick class (the health gate alone proves only `version` +
  fetch/verify, not that the daemon loop runs) without false-abandoning a target after one network
  sample.
...
The same bounded trade-off applies before probation: three consecutive controller fetch/verify
failures across supervised starts eventually abandon the target even when their root cause is an
extended external outage. Unlike the rc.12 behavior, a single failed request does not do so.
...
- Do not delete `state.json`: it also carries configuration, membership, apply-intent, and
  anti-rollback state. During a normal bounded health retry, leave `PendingUpdate` intact so the
  next supervised boot can retry or roll back safely.
- To re-arm an already rolled-back target (including an rc.12 node abandoned by one transient
  health request), stop the unit and atomically remove only `abandoned_agent_version` and
  `abandoned_reason` from `/var/lib/yaog-agent/state.json`, preserving every other JSON field plus
  the file owner and mode; then restart. If the unit supplies a custom `--state-dir`, use that path.
- To stop a rollout rather than retry it, remove the node from the rollout set (or clear the target)
  controller-side. Do not clear an in-flight breadcrumb without also reconciling which binary and
  `.bak` are installed.
