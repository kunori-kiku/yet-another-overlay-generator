...
- **A1 ✅** — the six original rc.1 blockers are fixed **with regression coverage**:
  - **B1** fleet-mux panic recovery → `internal/api/server.go:186` `recovered(mux)` wraps BOTH muxes,
    returns `apierr.CodeInternalPanic` (`:171`); regression: `internal/api/beta8_blockers_test.go` + the
    DAST live-wire path. (beta.8)
  - **S4/S5/S6** enrollment lifecycle → `enrollment.go:168-182,238` (`ErrNodeRevoked`/`reenrollApproved`),
...
- **A2 ✅** — **zero NEW confirmed blockers** from Subjects 1–4. The plan-21 (4.2) re-audit verdict
  ([`docs/spec/rc1/4.2-verdict.md`](docs/spec/rc1/4.2-verdict.md)) is **GO**: 4 findings, all LOW, all
...
| **B5** | `release.yml` gate is a **true byte-identical mirror** of `ci.yml` `go`/`frontend` (gofmt + `-race` + build) | `gate-go`/`gate-frontend` | ✅ (plan-22 Task 3) | n/a (tag-time) |
...
- **C1** — the **owed-smoke ledger** (canonical 8→3+1 reduction in
  [`docs/spec/rc1/RUNBOOK.md`](docs/spec/rc1/RUNBOOK.md)); live STATE below. Nine `STATUS.md` smokes
